...
const ContentConversation = () => {
    const {dataUserChat,conversationActive, userConversations,setConversationActive,initConversationActive} = useContext(ChatContext);
    const {messages, updatedAt} = conversationActive || {messages: [], updatedAt: new Date()};
    ...
    useEffect(() => {
        ...
        if(conversationActive?._id !== ''&&dataMessages.length===0||messages.length!==dataMessages.length){
            fetchDataMessages();
        }
    },[conversationActive])

    useEffect(() => {
      const findConversation = userConversations.find((conversation) =>conversation._id===conversationActive._id);
      if(!findConversation){
        setConversationActive(initConversationActive);
      }else{
        if(findConversation.messages.length!==0&&conversationActive.messages.length){
...
const formatDate = (date:Date) => {
    ...
    const result= new Intl.DateTimeFormat('es-MX', options)?.format(date as Date || new Date());
    return result;
}
